...
Pure stdlib helpers (no torch). Estimates actual writes (checkpoints, disk
caches, logs) plus a modest per-volume headroom. A previous version stacked a
flat 2 GiB reserve on *every* cost bucket and treated all POSIX paths as the
same volume (``Path.anchor`` is always ``/``), so a typical Anima LoRA was
rejected as needing ~4.4 GiB.
"""
...
SAFETY_FACTOR = 1.15
AUTOSAVE_MIN_FREE_BYTES = 64 * MiB
DEFAULT_FINETUNE_CHECKPOINT_BYTES = 12 * GiB
LORA_RESERVE_MIN_BYTES = 128 * MiB
LORA_RESERVE_MAX_BYTES = 512 * MiB
FINETUNE_RESERVE_MIN_BYTES = 512 * MiB
FINETUNE_RESERVE_MAX_BYTES = 2 * GiB
...
class DiskNeedEstimate:
    ...
    def with_safety(self) -> "DiskNeedEstimate":
        """Scale variable costs. Volume reserve is applied once per volume at check time."""

        def _scale(value: int) -> int:
            return int(math.ceil(value * SAFETY_FACTOR)) if value else 0

        scaled = DiskNeedEstimate(
            output_bytes=_scale(self.output_bytes),
            cache_bytes=_scale(self.cache_bytes),
            logging_bytes=_scale(self.logging_bytes),
            autosave_bytes=max(self.autosave_bytes, AUTOSAVE_MIN_FREE_BYTES),
            breakdown={k: int(v) if isinstance(v, (int, float)) else v for k, v in self.breakdown.items()},
...
def same_volume(path_a: str | os.PathLike[str] | None, path_b: str | os.PathLike[str] | None) -> bool:
    """True when both paths resolve onto the same mounted filesystem."""
...
def _periodic_saves(interval: int, total: int) -> int:
    """kohya keeps the last checkpoint; a periodic save on the last step is not extra."""
...
def _checkpoint_count(config: Mapping[str, object]) -> int:
    ...
    save_last_n = _int(config, "save_last_n_epochs", 0)

    # kohya default: only the final weights unless save_every_n_* is set.
    count = 1
    if save_every_epochs > 0 and max_epochs > 0:
        count += _periodic_saves(save_every_epochs, max_epochs)
    elif save_every_steps > 0 and max_steps > 0:
        count += _periodic_saves(save_every_steps, max_steps)
    if save_last_n > 0:
        count = min(count, save_last_n)
    return max(1, count)


def _lora_checkpoint_bytes(config: Mapping[str, object], model_train_type: str, network_dim: int) -> int:
    dim = max(1, network_dim)
    if model_train_type in ANIMA_LORA_TYPES:
        each = int(_clamp(dim * 5 * MiB, 24 * MiB, 1.5 * GiB))
        if _truthy(config.get("freeze_inserted_only_training")):
            each = max(16 * MiB, int(each * 0.35))
        return each
    if model_train_type in SD_FAMILY_LORA_TYPES or model_train_type.endswith("-lora"):
        return int(_clamp(dim * 3 * MiB, 16 * MiB, 1 * GiB))
    return int(_clamp(dim * 3 * MiB, 16 * MiB, 1 * GiB))
...
def _finetune_checkpoint_bytes(config: Mapping[str, object]) -> int:
    ...
    if model and os.path.isfile(model):
        try:
            return int(os.path.getsize(model) * 1.05)
        except OSError:
            pass
...
def _is_finetune(config: Mapping[str, object], model_train_type: str) -> bool:
    if model_train_type in FINETUNE_TYPES:
        return True
    if model_train_type == "anima-2.9b":
        return str(config.get("anima_29b_train_mode") or "lora").strip().lower() == "finetune"
    return False


def _volume_reserve(is_finetune: bool, variable_bytes: int) -> int:
    """One headroom per volume, scaled to the job — never a flat 2 GiB on LoRA."""
    variable = max(0, int(variable_bytes))
    if is_finetune:
        return int(_clamp(max(FINETUNE_RESERVE_MIN_BYTES, variable * 0.08), FINETUNE_RESERVE_MIN_BYTES, FINETUNE_RESERVE_MAX_BYTES))
    return int(_clamp(max(LORA_RESERVE_MIN_BYTES, variable * 0.20), LORA_RESERVE_MIN_BYTES, LORA_RESERVE_MAX_BYTES))
...
def estimate_training_disk_need(
    ...
    finetune = _is_finetune(config, model_train_type)
    ...
    if finetune:
        ...
        one = _lora_checkpoint_bytes(config, model_train_type, dim)
        output = one * saves
        breakdown["network_dim"] = dim
        breakdown["checkpoint_bytes_each"] = one
        if _truthy(config.get("save_state")):
            # AdamW moments dwarf the adapter; count ~3 extra copies of one ckpt.
            state = one * 3 * max(1, _int(config, "save_last_n_epochs_state", 1))
            output += state
            breakdown["save_state_bytes"] = state
    ...
    if _truthy(config.get("cache_latents_to_disk")):
        # Latent grid is typically /8 or /16; /16 * 16ch * bf16 ≈ a few hundred KB/image at 1024.
        latent = int(images * (w / 16.0) * (h / 16.0) * 16 * 2)
        latent *= tiers
        cache += latent
        breakdown["latents_cache_bytes"] = latent

    if _truthy(config.get("cache_text_encoder_outputs_to_disk")):
        if model_train_type in ANIMA_LORA_TYPES or model_train_type in {
            "anima-finetune",
            "anima-2.9b",
            "anima-2.9b-finetune",
            "krea2-lora",
    ...
    preview = _truthy(config.get("enable_preview")) or _truthy(config.get("sample_prompts"))
    misc = MISC_PREVIEW_BYTES if preview else MISC_LOGS_BYTES
    breakdown["logs_samples_margin_bytes"] = misc

    return DiskNeedEstimate(
        output_bytes=output + misc,
        cache_bytes=cache,
        logging_bytes=MISC_LOGS_BYTES if str(config.get("logging_dir") or "").strip() else 0,
...
def check_training_disk_space(
    ...
    *,
    ...
    output_variable = need.output_bytes + (need.cache_bytes if cache_on_output_volume else 0)
    output_required = output_variable + _volume_reserve(need.is_finetune, output_variable)
    need.breakdown["volume_reserve_bytes"] = output_required - output_variable
    ...
    if logging_dir and not same_volume(logging_dir, output_dir):
        log_variable = max(need.logging_bytes, MISC_LOGS_BYTES)
        checks.append(
            ("logging_dir", logging_dir, log_variable + _volume_reserve(False, log_variable))
        )
    if need.cache_bytes > 0 and train_data_dir and not cache_on_output_volume:
        checks.append(
            (
                "train_data_dir",
                train_data_dir,
                need.cache_bytes + _volume_reserve(False, need.cache_bytes),
            )
        )
        ...
        if free < required:
            raise DiskSpaceError(
                (
                    ...
                    f"（可在「训练器设置」关闭开训前磁盘检查，或设 {SKIP_ENV}=1）"
                ),
