Schema.intersect([
    Schema.object({
        model_train_type: Schema.string().default("anima-2.9b-finetune").hidden().description("训练种类"),
        pretrained_model_name_or_path: Schema.string().role('filepicker', { type: "model-file" }).default("./sd-models/anima/Anima-2.9B-preview-v1.safetensors").description("Anima 2.9B DiT 权重路径（40 层 preview，例如 Anima-2.9B-preview-v1.safetensors；全量微调将更新此 DiT 权重）"),
        freeze_inserted_only_training: Schema.boolean().default(true).description("只训插入层（pre1 配方）：冻结 28 个继承层，只训练 12 个交错插入块 2,5,8,11,14,17,21,24,27,30,33,36。需要 40 层 2.9B 检查点。关闭则训练全部已加载层"),
    ...
    Schema.intersect([
        ...
        Schema.union([
            Schema.object({
                ...
                sample_flow_shift: Schema.number().step(0.001).min(0.1).default(3.0).description("预览用 Rectified Flow 位移（写入 --fs）。默认 3.0；对齐 Comfy 时可试 5.0"),
