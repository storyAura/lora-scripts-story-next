Schema.intersect([
    ...
    Schema.intersect([
        Schema.object({
            ...
            lycoris_kernel_backend: Schema.union(["auto", "torch", "triton"]).default("auto").description("只影响 LyCORIS 算法（LoKr / LoHa / rsLoRA 等）的步进加速。推荐 auto：能用 Triton 就用，不行自动回退。要强制 Triton 选 triton；数值异常再改回 torch。原生 LoRA / T-LoRA 以及 bokr / bora / gsokr / glora_boft 不走这里"),
