...
- **修复** `disk_preflight_enabled`：开关原先只在首次整页打开设置页时接线，从首页点进 `/other/settings.md` 后不会写入 `trainer_settings.json`，界面常显示关闭但开训仍预检。现随 SPA 导航生效，以本机设置为准，`/api/run` 与队列启动都会读这个开关。
